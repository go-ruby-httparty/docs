...
`go-ruby-httparty/httparty` reimplements Ruby's `httparty` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-httparty/httparty` directly, with no Ruby runtime;
...
Because `httparty` is pure Go with cgo disabled, it also compiles to **WebAssembly**
